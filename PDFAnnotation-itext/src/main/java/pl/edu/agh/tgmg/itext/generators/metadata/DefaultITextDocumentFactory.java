...
public class DefaultITextDocumentFactory implements ITextDocumentFactory {
    ...
    public Document create(OutputStream out, DocumentMetaData metaData) throws GenDocumentException {
        try {
            Document document = new Document(metaData.getPageSize());
            ...
            document.setMargins(metaData.getMargins().getForLeft(), 
                    metaData.getMargins().getForRight(), 
                    metaData.getMargins().getForTop(), 
                    metaData.getMargins().getForBottom());
